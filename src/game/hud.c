...
void render_hud_tex_lut(s32 x, s32 y, u8* texture) {
  ...
  gSPTextureRectangle(gDisplayListHead++, x << 2, y << 2, (x + 15) << 2,
                      (y + 15) << 2,
...
void render_hud_small_tex_lut(s32 x, s32 y, u8* texture) {
  ...
  gDPSetTile(gDisplayListHead++, G_IM_FMT_RGBA, G_IM_SIZ_16b, 2, 0,
             G_TX_RENDERTILE, 0,
  ...
  gSPTextureRectangle(gDisplayListHead++, x << 2, y << 2, (x + 7) << 2,
                      (y + 7) << 2, G_TX_RENDERTILE,
                      0, 0, 4 << 10, 1 << 10);
}
...
void animate_power_meter_emphasized(void) {
  ...
  else { sPowerMeterVisibleTimer = 0; }
}
...
static void animate_power_meter_deemphasizing(void) {
  ...
  if (sPowerMeterHUD.y >= 181) { speed = 3; }

  if (sPowerMeterHUD.y >= 191) { speed = 2; }

  if (sPowerMeterHUD.y >= 196) { speed = 1; }
...
void handle_power_meter_actions(s16 numHealthWedges) {
  ...
  if (numHealthWedges < 8 && sPowerMeterStoredHealth == 8 && sPowerMeterHUD.
...
void render_hud_mario_lives(void) {
  ...
  text_renderer.render()
               ->scheduled()
               ->glyph_at(
                 GLYPH_MARIO_HEAD, GFX_DIMENSIONS_RECT_FROM_LEFT_EDGE(22),
                 224 - HUD_TOP_Y)
               ->glyph_at(
                 GLYPH_MULTIPLY, GFX_DIMENSIONS_RECT_FROM_LEFT_EDGE(38),
                 224 - HUD_TOP_Y);
...
void render_hud_coins(void) {
  ...
  text_renderer.render()
               ->scheduled()
               ->glyph_at(GLYPH_COIN, 168, 224 - HUD_TOP_Y);
...
void render_hud_stars(void) {
  ...
  text_renderer.render()
               ->scheduled()
               ->glyph_at(
                 GLYPH_STAR, GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(HUD_STARS_X),
                 224 - HUD_TOP_Y);

  //print_text(GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(HUD_STARS_X), HUD_TOP_Y, "-"); // 'Star' glyph
  if (showX == 1) {
    print_text(
      GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(HUD_STARS_X) + 16, HUD_TOP_Y, "*");
    // 'X' glyph
  }
  print_text_fmt_int(
    (showX * 14) + GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(HUD_STARS_X - 16),
...
void render_hud_timer(void) {
  ...
  timerFracSecs = ((timerValFrames - (timerMins * 1800) - (timerSecs * 30)) &
                   0xFFFF) / 3;
#ifndef VERSION_EU
  print_text(GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(150), 185, "TIME");
#endif
  print_text_fmt_int(
    GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(91), 185, "%0d", timerMins);
  print_text_fmt_int(
    GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(71), 185, "%02d", timerSecs);
  print_text_fmt_int(
    GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(37), 185, "%d", timerFracSecs);
  gSPDisplayList(gDisplayListHead++, dl_hud_img_begin);
  render_hud_tex_lut(
    GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(81), 32, (*hudLUT)[GLYPH_APOSTROPHE]);
  render_hud_tex_lut(
    GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(46), 32, (*hudLUT)[GLYPH_DOUBLE_QUOTE]);
 ...
 */
void set_hud_camera_status(s16 status) { sCameraHUD.status = status; }
...
void render_hud_camera_status(void) {
  ...
  switch (sCameraHUD.status & CAM_STATUS_C_MODE_GROUP) {
    case CAM_STATUS_C_DOWN:
      render_hud_small_tex_lut(x + 4, y + 16,
                               (*cameraLUT)[GLYPH_CAM_ARROW_DOWN]);
      break;
...
void render_hud(void) {
  ...
  if (hudDisplayFlags == HUD_DISPLAY_NONE) {
    ...
    if (gCurrentArea != NULL && gCurrentArea->camera->mode ==
        CAMERA_MODE_INSIDE_CANNON) { render_hud_cannon_reticle(); }
