...
const LevelScript* get_level_main_scripts_entry() {
  ...
  int script_count;
  auto scripts = DynamicLevelScriptBuilder()
...
std::shared_ptr<DynamicLevelScriptBuilder> get_script_exec_level_table() {
  auto builder = new DynamicLevelScriptBuilder();
  ...
  return std::shared_ptr<DynamicLevelScriptBuilder>(builder);
}
...
#define DEFINE_LEVEL(_0, _1, _2, folder, _4, _5, _6, _7, _8, _9, _10) \
   ...
   const u8* _##folder##SegmentRomEnd_ = nullptr; \
            \
  std::shared_ptr<DynamicLevelScriptBuilder> get_script_exec_##folder() {    \
    auto script_exec = new DynamicLevelScriptBuilder();                      \
    ...
    script_exec->add_script(RETURN());                         \
                                                                      \
    return std::shared_ptr<DynamicLevelScriptBuilder>(script_exec);          \
  }
#include "processed_level_defines.h"
#undef DEFINE_LEVEL
